...
describe('RegisterComponent', () => {
  ...
  O snackbar vai abrir com mensagem de sucesso`, () => {
    ...
    const btn = fixture.nativeElement.querySelector('.button--register');
    btn.disabled = false;
    btn.click();

    expect(matSnackBar.open).toHaveBeenCalledWith('Sua conta foi criada', 'Fechar', { duration: 3000 });
  });
  ...
  O snackbar vai abrir com mensagem de erro`, () => {
    spyOn(authService, 'signUp').and.returnValue(Promise.reject(new Error('fail')));
    spyOn(matSnackBar, 'open');
